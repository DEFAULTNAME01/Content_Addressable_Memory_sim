...
DEFINE_SELECTIVE_READ_FIRST(RoB_ITEM,ReorderBuffer);
...
int main() {
   ...
   // REGISTER_PURGE_WITH_CONDITION(ReorderBuffer,RoB,predicateIndexPURGE_BY,&value)
    ...
    //RoB_ITEM *A = ReorderBuffer_SELECTIVE_READ_FIRST(&RoBDestR,predicateIndexMATCH_BY,&value);
    RoB_ITEM *A = REGISTER_S_R_F(ReorderBuffer,RoB,predicateIndexMATCH_BY,&value);
    printf("SR matched OpCode: %d, operand1: %d, operand2: %d, srcPCindex: %d, targetRFindex: %d, Result: %d, ispredicate: %d,\n isreadyforcommit: %d, targetMemindex: %d\n   predicateIndex: %d\n",
            A->opcode, A->operand1, A->operand2,
           A->srcPCindex, A->targetRFindex, A->Result,
           A->ispredicate, A->isreadyforcommit, A->targetMemindex, A->predicateIndex );

    if (A != NULL) {
    A->Result = A->operand1+A->operand2;
    A->isreadyforcommit=1;  // 直接对结构体中的字段进行赋值
    }
    printf("SR after OpCode: %d, operand1: %d, operand2: %d, srcPCindex: %d, targetRFindex: %d, Result: %d, ispredicate: %d,\n isreadyforcommit: %d, targetMemindex: %d\n   predicateIndex: %d\n",
            A->opcode, A->operand1, A->operand2,
           A->srcPCindex, A->targetRFindex, A->Result,
           A->ispredicate, A->isreadyforcommit, A->targetMemindex, A->predicateIndex );
    // 出队并打印
...
while (REGISTER_dequeue(ReorderBuffer, RoB, &dequeuedItem) == 0) {
    printf("REGISTER Dequeued%d OpCode: %d, operand1: %d, operand2: %d, srcPCindex: %d, targetRFindex: %d, Result: %d, ispredicate: %d,\n isreadyforcommit: %d, targetMemindex: %d\n   predicateIndex: %d\n",
