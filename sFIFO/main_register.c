...
int main() {
    ...
    REGISTER_enqueue(IssueQueueFIFO,myFifo,iq1);
    ...
    REGISTER_enqueue(IssueQueueFIFO,myFifo,iq2);
    // 创建并初始化第三个IssueQueue项
    IssueQueue *iq3 = malloc(sizeof(IssueQueue));
    iq3->Opcode = ADD;
    iq3->s1 = 7;
    iq3->s2 = 8;
    iq3->r = 9;
    // IssueQueueFIFO_enqueue(&myFifoSrcR, iq3);
    REGISTER_enqueue(IssueQueueFIFO,myFifo,iq3);
    ...
    if (IssueQueueFIFO_dequeue(&myFifoDestR, &dequeuedItem) == 0) {
        printf("myR Dequeued OpCode: %d, s1: %d, s2: %d, r: %d\n", dequeuedItem->Opcode, dequeuedItem->s1, dequeuedItem->s2, dequeuedItem->r);
        free(dequeuedItem);
    }
   
    if ( REGISTER_dequeue(IssueQueueFIFO,myFifo, &dequeuedItem) == 0) {
        printf("myR Dequeued2 OpCode: %d, s1: %d, s2: %d, r: %d\n", dequeuedItem->Opcode, dequeuedItem->s1, dequeuedItem->s2, dequeuedItem->r);
        free(dequeuedItem);
    }
    if ( REGISTER_dequeue(IssueQueueFIFO,myFifo, &dequeuedItem) == 0) {
        printf("myR Dequeued3 OpCode: %d, s1: %d, s2: %d, r: %d\n", dequeuedItem->Opcode, dequeuedItem->s1, dequeuedItem->s2, dequeuedItem->r);
        free(dequeuedItem);
    }
